...
class MapDataset(dg.MultiSlotDataGenerator):
    ...
    def _process_line(self, line):
        ...
        for fea in self.pid_list:
            context_feature.append([mmh3.hash(fea + str(instance[fea])) % self.hash_dim])
            context_feature_fm.append(mmh3.hash(fea + str(instance[fea])) % self.hash_dim)
        for fea in self.query_feature_list:
            context_feature.append([mmh3.hash(fea + str(query[fea])) % self.hash_dim])
            context_feature_fm.append(mmh3.hash(fea + str(query[fea])) % self.hash_dim)
        for fea in self.plan_feature_list:
            context_feature.append([mmh3.hash(fea + str(plan[fea])) % self.hash_dim])
            context_feature_fm.append(mmh3.hash(fea + str(plan[fea])) % self.hash_dim)
        ...
        for fea in self.weather_feature_list:
            context_feature.append([mmh3.hash(fea + str(weather_dic[fea])) % self.hash_dim])
            context_feature_fm.append(mmh3.hash(fea + str(weather_dic[fea])) % self.hash_dim)
    ...
    def generate_sample(self, line):
        def data_iter():
            ...
            yield list(zip(feature_name, [dense_feature] + sparse_feature + [sparse_feature_fm] + [label]))
